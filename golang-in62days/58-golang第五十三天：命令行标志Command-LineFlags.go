...
golang第五十三天:go语言中的命令行标志Command-Line Flags
http://www.kuaizh.com/?p=771


命令行标志（使用标记的命令行处理方法，命令行标志是命令行程序指定选项的常用方式。例如，在 wc -l 中，这个 -l 就是一个命令行标志。）
...
func  go build command-line-flags.go

//给所有的标示设置值
func  ./command-line-flags -word=opt -numb=7 -fork -svar=flag
word: opt
numb: 7
fork: true
svar: flag
...
func  ./command-line-flags -word=opt
word: opt
numb: 42
fork: false
svar: bar
...
//位置参数可以出现在任何标志后面。
func  ./command-line-flags -word=opt a1 a2 a3
word: opt
...
tail: [a1 a2 a3]

//注意，flag 包需要所有的标志出现位置参数之前（否则，这个标志将会被解析为位置参数）。
func  ./command-line-flags -word=opt a1 a2 a3 -numb=7
word: opt
numb: 42
fork: false
svar: bar
